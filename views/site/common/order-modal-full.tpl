            ...
            <form id="main-order" method="POST">
                ...
                <div class="product product-one global-pr<?=$kidsID?>">
                    ...
                    <div class="product-name">
                        ...
                        <input type="hidden" name="pr-id[]" value="<?=$kidsID?>">
                    </div>
                    <div class="form-group numeric">
                        <input class="qty<?=$kidsID?>" type="text" name="qty[]" id="qty<?=$kidsID?>" value="0" disabled>
                        <input class="qty1<?=$kidsID?>" type="hidden" name="qty1[]>" id="qty1<?=$kidsID?>" value="0">
                        <span class="inc inc<?=$kidsID?>">+</span>
                        <span class="dec dec<?=$kidsID?>">-</span>
                    </div>                
                    <div class="price">
                        <span class="price-i<?=$kidsID?>"><?= $price ?></span> <span><?= Yii::t('yii', 'UAH') ?></span>
                        <input id="price<?=$kidsID?>" name="price[]" type="hidden" value="<?= $price ?>">
                    </div>
                    <div class="price-total total<?=$kidsID?>">
                        <span  class="price-i">0</span> <span><?= Yii::t('yii', 'UAH') ?></span>
                        <input id="total<?=$kidsID?>" name="total[]" type="hidden" value="0">
                    </div>
                </div>
                ...
                <div class="product product-eng global-pr<?=$pr['id']?>">
                    <div class="product-img">
                        ...
                        <input type="hidden" name="pr-image[]" value="<?= $url.$pr['images'][0]['src'] ?>">
                    </div>
                    <div class="product-name">
                        <p><?= $pr['name'] ?></p>
                        <input type="hidden" name="pr-name[]" value="<?= $pr['name'] ?>">
...
function validatePromo(code){
    if (code == "ES_promo_2018" || code == "5107"){
        return true;
    } else {
        return false;
    }
...
var nameCustom = $(".product-one .product-name p").text();

var skus= [nameCustom, "A1", "A2", "B11", "B12", "B21", "B22", "C1", "C2"];
...
submitHandler: function(form) {
			...
			$('#order-modal-full button').attr('disabled', true).empty().html('<i class="fa fa-spinner" aria-hidden="true"></i>');
                ...
                success: function(response) {
                    ...
                    //},250);
                    //$('#close-modal-full').trigger('click');
                    $('#order-modal-full').innerHTML = response;
                    setTimeout(function(){
